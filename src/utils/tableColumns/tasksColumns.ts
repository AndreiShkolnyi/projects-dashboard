...
export const columns: ColumnDef<TasksWithProjects[0]>[] = [
  {
    ...
    cell: ({ row }) => {
      return h(RouterLink, { to: `/tasks/${row.original.id}` ,class: 'text-left font-medium hover:bg-muted block w-full hover:text-blue-500' }, () => row.getValue('name'))
    },
  },
  {
    accessorKey: 'status',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return  h(RouterLink, { to: `/projects/${row.original.projects?.slug}` ,class: 'text-left font-medium hover:bg-muted block w-full hover:text-blue-500' }, 1)
    }
  },
  {
