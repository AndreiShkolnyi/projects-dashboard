...
import { profilesQuery, projectsQuery, createNewTaskQuery } from '@/utils/supabaseQueries';
  ...
  <Sheet v-model:open="sheetOpen">
    <SheetContent>
      ...
      <FormKit type="form" @submit="createTask" submit-label="Create Task" :config="{
        ...
        <FormKit type="select" name="profile_id" id="profile_id" label="User" placeholder="Select a user"
          :options="selectOptions.profiles" />
        <FormKit type="select" name="project_id" id="project_id" label="Porject" placeholder="Select a project"
          :options="selectOptions.projects" validation="required" />
        <FormKit type="textarea" name="description" id="description" label="Description" placeholder="Task description"
